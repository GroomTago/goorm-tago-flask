...
class TaxiReservation(db.Model):
    ...
    starting_point_latitude = db.Column(db.Numeric(precision=10, scale=8), nullable=False)  # 출발지 위도
    starting_point_longitude = db.Column(db.Numeric(precision=11, scale=8), nullable=False) # 출발지 경도
    arrival_point = db.Column(db.String(255), nullable=False)
    arrival_point_latitude = db.Column(db.Numeric(precision=10, scale=8), nullable=False)   # 도착지 위도
    arrival_point_longitude = db.Column(db.Numeric(precision=11, scale=8), nullable=False)  # 도착지 경도
